...
Перед запуском тестов рекомендуется полная лчистка кэша приложения. Для этого воспользуйтесь Ассистентом приложения. Обязательным пунктом, для успешного запуска тестов, является _Кэш конфигураций_.
