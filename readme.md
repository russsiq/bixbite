...
Ссылки на некоторые ресурсы носят субъективный характер.
...
При запросе `Do you want to remove the existing VCS (.git, .svn..) history? [Y,n]?`, выбрать `n` для сохранения системы контроля версий. Аргументы `--no-bin-links` при использовании операционной системы ***Windows***.
...
## Несколько слов о фреймворке Laravel
