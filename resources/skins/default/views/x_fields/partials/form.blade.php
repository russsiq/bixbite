<div class="card card-default">
	...
	<div class="card-body">
        ...
        <div class="form-group row {{ $errors->has('extensible') ? ' has-error' : '' }}">
			...
			<div class="col-sm-5">
				@if (optional($x_field)->id)
					<input type="text" name="extensible" class="form-control" required readonly
	                	value="{{ optional($x_field)->extensible }}" />
				@else
					<select name="extensible" class="form-control" required {{ optional($x_field)->id ? 'readonly' : '' }}>
						@foreach($extensibles as $extensible)
						    <option value="{{ $extensible }}"
						        @if ($extensible == old('extensible', optional($x_field)->extensible))
									selected
								@endif
								>{{ $extensible }}</option>
							{{-- @lang($extensible) --}}
		...
		<div class="form-group row {{ $errors->has('type') ? ' has-error' : '' }}">
			...
			<div class="col-sm-5">
				@if (optional($x_field)->id)
					<input type="text" name="type" class="form-control" required readonly
	                	value="{{ optional($x_field)->type }}" />
				@else
					<select name="type" class="form-control" required {{ optional($x_field)->id ? 'readonly' : '' }}>
		...
		<div class="form-group row {{ $errors->has('params') ? ' has-error' : '' }}">
			...
			<div class="col-sm-5">
				...
				{{--
					...
					@endif
				--}}
				<textarea name="params" rows="4" class="form-control"
					{{ optional($x_field)->id ? 'readonly' : '' }}
					>{{ old('params', optional($x_field)->getOriginal('params')) }}</textarea>
			</div>
		</div>
	</div>
</div>
