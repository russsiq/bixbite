...
export default {
    ...
    computed: {
        ...
        nestedCategories() {
            ...
            const roots = categories.filter(
                (element) => !element.attributes.parent_id
            );
        ...
        getNestedCategories() {
            return (items, depth, categories) => {
                ...
                items.forEach((item, index, array) => {
                    ...
                    item.selected = this.$props.value.some(
                        (category) => item.id === category.id
                    );

                    item.title =
                        " · ".repeat(item.depth) + item.attributes.title;

                    nested.push(item);

                    const children = categories.filter(
                        (element) => element.attributes.parent_id === item.id
                    );
    ...
    created() {
        Category.$fetch({}).then(this.fillForm);
    },
