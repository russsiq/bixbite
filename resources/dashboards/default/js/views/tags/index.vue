<template>
    <filterable v-bind="filterable" :collection.sync="collection">
        ...
        <template #second-group>
            <router-link :to="{name: 'tags.create'}" class="btn btn-outline-success">Create</router-link>
        </template>
            ...
            <tr :key="row.id">
                ...
                <td class="text-end">
                    ...
                    >
                        ...
                        <router-link
                            :to="{name: 'tags.edit', params: {id: row.id}}"
    ...
    methods: {
        destroy(tag) {
            const result = confirm(
                `Do you want to remove this tag [${tag.attributes.title}]?`
            );
